...
//     // const reviewedUser=await ReviewSchema.find({User:req.user,Product:ProductId})
//     // if(reviewedUser){
//     //     return res.status(302).json({msg:"User has already reviewed  the product"})
//     // }

//     const{Comments,Rating}=req.body
//     // const ReviewedUser=await ReviewSchema()
//     if(!Comments||!Rating){
//         return res.status().json({msg:"One or mandatory field is empty"})
//     }
//     const  reviewdata={userId:req.user, rating:Rating,comment:Comments}
//      ProductData.ratings.push(reviewdata)
//     const review= await ProductData.save()
...
//  return res.status(201).json({msg:"reviewed by the User",review:review})
//  //return res.status(201).json({ msg: "reviewed by the User", review: ReviewData });
// }
exports.AddReview = async (req, res, next) => {
  const ProductId = req.params.ProductId;
  const ProductData = await ProductSchema.findById(ProductId);
  ...
  try {
    ...
    const foundObject = ProductData.ratings.find((obj) => obj.userId.toString() === req.user._id.toString());
    ...
    if (!Comments || !Rating) {
      return res.status(400).json({ msg: "One or more mandatory fields are empty" });
    }
    ...
    return res.status(201).json({ msg: "Review added successfully",data: ProductData.ratings,foundReview });
  } catch (error) {
    console.error(error);
...
exports.UpdateReview = async (req, res, next) => {
  const ProductId = req.params.ProductId
  const { Comments, Rating } = req.body
  const reviewData = await ReviewSchema.findOneAndUpdate({ Product: ProductId }, {
    Comments: Comments,
    Rating: Rating
  }, {
    new: true
...
exports.reviewforfrontend = async (req, res, next) => {
  try {
    const ProductId = req.params.ProductId;
    const ExistReview = await ReviewSchema.find({Product:ProductId}).populate('User.userid','Name')

    if (!ExistReview) {
      return res.status(404).json({ msg: "Review not found" });
    }

    // Extract user IDs and comments from the ratings array
    // const reviewArray = await ReviewSchema.aggregate([
    //   {
    //     $match: { _id: ExistReview._id }, // Match the specific product
    //   },
