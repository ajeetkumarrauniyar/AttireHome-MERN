...
exports.Register = async (req, res, next) => {
    try {
        ...
        transporter.sendMail({
            ...
            html:"<h1>thanx for the Signup in AttireHome </h1>"
        })   
...
exports.Login= async (req, res, next) => {
    try {
        ...
        const userInfo = ({ "Email": user.Email, "Name": user.Name, "_id": user._id ,"IsAuth":user.IsAuth})
...
try{
    ...
    const user=await User.findById({_id:userId},'Name Email Address Phone')
    if(!user){
...
exports.resetPassword = async (req, res) => {
    try {
        ...
        res.status(200).json({ msg: 'Password reset successful', user: updatedUser });
    } catch (error) {
        console.error(error);
