...
exports.addToCart = async (req, res, next) => {
   try{
    ...
    if (CartUser) {
        ...
        let existProduct = req.user.Cart.Items.find(p => p.Product.toString() === productId.toString())
        if (existProduct) {
         
            let existIndex = req.user.Cart.Items.findIndex(p => p.Product.toString() === productId.toString())
            console.log(existIndex)
            const ud = req.user.Cart.Items[existIndex].Quantity += 1
            existProduct = await req.user.save()
            // console.log(ud) 
            return res.status(201).json({"cart": CartUser ,"UserCartdata" : req.user.Cart})
        }
        const Userdata = {
            Product: productId,
            Quantity: Quantity
...
exports.deleteProductsFromCart = async (req, res, next) => {
    ...
    try {
        ...
        const existProductIndex = cart.Product.findIndex(p => p._id.toString() === productId.toString());
        ...
        if (existProductIndex !== -1) {
            ...
            if (req.user.Cart && req.user.Cart.Items) {
                const UserCartIndex = req.user.Cart.Items.findIndex(item => item.Product.toString() === productId.toString());
                if (UserCartIndex !== -1) {
    ...
    try{
        ...
        if (CartUser) {
            ...
            if (existProduct) {
                ...
                existProduct = await req.user.save()
                // console.log(ud) 
                return res.status(201).json({msg:"product increased",Items:req.user.Cart.Items[existIndex]})
            }
    ...
    try{
        ...
        if (CartUser) {
            ...
            if (existProduct.Quantity >1) {
                ...
                existProduct = await req.user.save()
                // console.log(ud) 
                return res.status(201).json({msg: "product decreases", Items:req.user.Cart.Items[existIndex] })
            }
            
         return res.status(502).json({msg:"you can't decrease the Quantity from now"})
        }
...
exports.CartbyUserId = async (req, res, next) => {
  try{
    let amount=0;
    let  totalitems=0;
   const  UserCart= await CartModel.findOne({User:req.user}).populate("Product", "Name Price Quantity  ImageUrl Category._id")
 ...
 //   console.log(totalitems)
      ...
      return res.status(200).json({"UsersItems":UserCart ,
      "dataInUserM":data,
      "Amount":amount,
